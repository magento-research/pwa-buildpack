...
class WebpackPEM {
    ...
    constructor(
        p = this.constructor.DEFAULT_PATH,
        {
            keyBlockName = this.constructor.KEY_BLOCK_NAME,
            certBlockName = this.constructor.CERT_BLOCK_NAME
        } = {}
    ) {
        this.exists = false;
        this.key = undefined;
        ...
        this.contents = '';
        this.path = p;
        this._parse = this.constructor.BlockParser({
            key: keyBlockName,
            cert: certBlockName
        });
    }
    ...
    write({ key, cert } = {}) {
        ...
        if (!parsed.key || !parsed.cert) {
            throw Error('Unrecognized input passed to webpackPem.write().');
        }
